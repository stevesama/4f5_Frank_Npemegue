...
/**
 * Precompiled local-jsweet-plugin script plugin.
 **/
public class LocalJsweetPluginPlugin implements org.gradle.api.Plugin<org.gradle.api.internal.project.ProjectInternal> {
    ...
    public void apply(org.gradle.api.internal.project.ProjectInternal target) {
        assertSupportedByCurrentGradleVersion();
                target.getPluginManager().apply("org.jsweet.jsweet-gradle-plugin");
        target.getPluginManager().apply("maven");

        try {
            Class<? extends BasicScript> precompiledScriptClass = Class.forName("precompiled_LocalJsweetPlugin").asSubclass(BasicScript.class);
